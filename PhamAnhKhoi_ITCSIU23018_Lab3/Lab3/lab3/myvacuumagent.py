...
class MyVacuumAgent(Agent):
    ...
    def __init__(self, world_width, world_height, log):
        ...
        self.iteration_counter = world_width * world_height * 5
    ...
    def best_fs(self, return_home=False):
        start = (self.state.pos_x, self.state.pos_y)
        queue = []
        explored = set()
        ...
        while queue:
            ...
            for dx, dy in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                ...
                child = (nx, ny)
                if child in explored or any(child == n for _, n, _ in queue):
                    continue

                priority = self.heuristic(child, self.home_pos if return_home else (nx, ny))
                heapq.heappush(queue, (priority, child, path + [child]))
                
        self.node_explored = len(explored)
        
    def a_star(self, return_home=False):
        start = (self.state.pos_x, self.state.pos_y)
        queue = []
        explored = set()
        g_scores = {start: 0}
        ...
        while queue:
            ...
            for dx, dy in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                ...
                g_scores[child] = tentative_g_score
                priority = tentative_g_score + self.heuristic(child, self.home_pos if return_home else (nx, ny))
                heapq.heappush(queue, (priority, child, path + [child]))
    ...
    def execute(self, percept):
        ...
        if not self.route:
            ...
            # self.best_fs(return_home=False)
            self.a_star(return_home=False)
            ...
            if not self.route:
                ...
                # self.best_fs(return_home=True)
                self.a_star(return_home=True)
